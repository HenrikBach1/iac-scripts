...
# SOLUTION: Use OpenSSL to generate PKCS#8 format keys for maximum libssh2 compatibility
echo "Generating SSH keys in PKCS#8 format (optimal for guacd/libssh2)..."

# This is the key fix: OpenSSL generates keys in PKCS#8 format which is more compatible
# with libssh2 than the traditional PEM format generated by ssh-keygen -m PEM
...
# Verify the key format - should be PKCS#8 format
if head -1 /etc/guacamole/guaczero_rsa | grep -q "BEGIN PRIVATE KEY"; then
    echo "✅ SUCCESS: SSH key is in PKCS#8 format (optimal for libssh2)"
    echo "Key header: $(head -1 /etc/guacamole/guaczero_rsa)"
else
    echo "❌ WARNING: Key format might not be optimal for libssh2"
    echo "Key header: $(head -1 /etc/guacamole/guaczero_rsa)"
fi
...
echo "   • PKCS#8 SSH key format (libssh2 compatible)"
...
echo "   • Generated with: OpenSSL (not ssh-keygen)"
...
if [ "$WORKING_METHODS" -gt 0 ]; then
    echo ""
    echo "✅ SUCCESS: Zero-trust SSH with PKCS#8 key format is working!"
